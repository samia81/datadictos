...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="blog">
		...
		<xsl:for-each select="comment"><xsl:sort select="date" order="ascending"/>
			...
			<xsl:text>نشرت </xsl:text><xsl:value-of select="substring(date,7,2)" />/<xsl:value-of select="substring(date,5,2)" />/<xsl:value-of select="substring(date,1,4)" /> - <xsl:value-of select="substring(date,9,2)" />:<xsl:value-of select="substring(date,11,2)" />
			<xsl:text> قبل </xsl:text>
			<strong>
			<xsl:choose>
				<xsl:when test="url != ''"> 
